...
from crew_definition import MarketResearchCrew
...
async def execute_crew_task(input_data: dict) -> str:
    """ Execute a MarketResearch task. `input_data` should be a dict
    containing `coingecko_id` (or `token`) and optional fields.
    """
    logger.info(f"Starting MarketResearch task with input: {input_data}")
    crew = MarketResearchCrew(logger=logger)
    result = crew.crew.kickoff(input_data)
    logger.info("MarketResearch task completed successfully")
    return result


def summarize_report(result_obj) -> str:
    """Return a compact human-readable summary for a MarketResearch report.

    Accepts the `CrewShim.Result` object or the raw dict.
    """
    raw = result_obj.raw if hasattr(result_obj, "raw") else result_obj
    if not isinstance(raw, dict):
        # Fallback: pretty-print whatever we received
        try:
    ...
    parts = []
    cid = raw.get("coingecko_id") or raw.get("id") or "<unknown>"
    parts.append(f"Asset: {cid}")

    price = raw.get("price", {})
    if price:
        cp = price.get("current_price")
        c24 = price.get("change_24h_pct")
        c7 = price.get("change_7d_pct")
        parts.append(f"Current price: {cp} (24h: {c24:.2f}%  7d: {c7:.2f}% )" if cp is not None and c24 is not None and c7 is not None else f"Price data: {price}")

    news = raw.get("news_headlines", [])
    if news:
        parts.append(f"News headlines ({len(news)}): {news[:3]}")

    news_sent = raw.get("news_sentiment")
    if news_sent:
        parts.append(f"News sentiment score: {news_sent.get('score')} (+{news_sent.get('positive')}/-{news_sent.get('negative')}/~{news_sent.get('neutral')})")
    ...
    if rec:
        parts.append(f"Recommendation: {rec.get('action').upper()} (confidence: {rec.get('confidence')})")
...
async def start_job(data: StartJobRequest):
    ...
    try:
        ...
        # Log an informative representation of the input (token or text)
        input_text = data.input_data.get("coingecko_id") or data.input_data.get("token") or data.input_data.get("text") or "<no_text>"
        truncated_input = str(input_text)[:100] + "..." if len(str(input_text)) > 100 else str(input_text)
        ...
        "input_data": [
            {
                "id": "coingecko_id",
                "type": "string",
                "name": "Token Identifier (CoinGecko ID)",
                "data": {
                    "description": "The CoinGecko ID for the token/asset (e.g., 'cardano')",
                    "placeholder": "cardano"
                }
            },
            {
                ...
                "type": "string",
                "name": "Quote Currency",
                "data": {"description": "Currency to compare against (default 'usd')", "placeholder": "usd"}
            },
            {
                "id": "news_api_key",
                "type": "string",
                "name": "Optional NewsAPI Key",
                "data": {"description": "Optional API key to enrich analysis with news headlines"}
            }
        ]
    }
...
def main():
    ...
    # Define test input for market research
    input_data = {"coingecko_id": "cardano", "vs_currency": "usd"}

    print(f"Input: {input_data}")
    print("\nProcessing with MarketResearch agents...\n")
    
    # Initialize and run the crew
    crew = MarketResearchCrew(verbose=True)
