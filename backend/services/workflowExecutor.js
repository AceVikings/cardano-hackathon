...
function resolveNodeInputs(node, edges, nodeOutputs) {
  ...
  const configuredValues = nodeData.inputValues || {};
  ...
  for (const param of inputParameters) {
    ...
    } else if (configuredValues[param.name]) {
      // Use manually configured value
      inputs[param.name] = configuredValues[param.name].value;
    }
  }
...
async function executeAgentNode(node, inputs, context) {
  ...
  // In a real implementation, this would make an HTTP call to the agent service
  ...
  const agentType = node.data?.agentType || 'custom';
  ...
  switch (agentType) {
    case 'swap':
      return {
        success: true,
        output: {
          toTokenAmount: inputs.totalAmount ? inputs.totalAmount * 1.5 : 0,
 ...
 */
async function executeWorkflow(workflowId, triggerType, triggerData = {}, context = {}) {
  const executionId = `exec_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  const startTime = Date.now();
  
  console.log(`[Executor] Starting execution ${executionId} for workflow ${workflowId}`);
  ...
  if (context.userId && workflow.userId.toString() !== context.userId.toString()) {
  ...
  if (triggerType !== TriggerType.MANUAL) {
    ...
    if (workflowTriggerType !== triggerType) {
      throw new Error(`Trigger type mismatch: expected ${workflowTriggerType}, got ${triggerType}`);
    }
  }
  ...
  for (const node of executionOrder) {
    ...
    try {
      if (node.type === 'trigger') {
        // Trigger nodes don't execute, they just pass through trigger data
        nodeOutputs.set(node.id, { triggerData, triggered: true });
  ...
  } else if (failedNodes === nodeResults.filter(n => n.nodeType === 'agent').length) {
  ...
  await Workflow.findByIdAndUpdate(workflowId, {
    $inc: {
      ...
      'stats.successfulExecutions': overallStatus === ExecutionStatus.SUCCESS ? 1 : 0,
      'stats.failedExecutions': overallStatus === ExecutionStatus.FAILED ? 1 : 0,
    },
    $set: {
  ...
  const executionResult = {
    ...
    summary: {
      ...
      successfulNodes: nodeResults.filter(n => n.status === ExecutionStatus.SUCCESS).length,
      failedNodes,
    },
    timing: {
  ...
  console.log(`[Executor] Execution ${executionId} completed with status: ${overallStatus}`);
...
async function validateWorkflow(workflowId, context = {}) {
  ...
  const disconnectedAgents = agentNodes.filter(n => !connectedAgents.has(n.id));
  if (disconnectedAgents.length > 0) {
    return { 
      valid: false, 
      error: `Some agent nodes are not connected: ${disconnectedAgents.map(n => n.data?.label).join(', ')}` 
    };
  }
