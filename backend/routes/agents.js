...
router.post("/conversation-agent/invoke", async (req, res) => {
  try {
    ...
    const agentHost = process.env.AGENT_SERVICE_HOST || "http://139.84.155.199:8000";
    const purchaseUrl = process.env.AGENT_PURCHASE_URL || "http://139.84.155.199:3001/api/v1/purchase/";
    ...
    console.error("Conversation agent error:", error.response?.data || error.message);
    res.status(500).json({
      success: false,
  ...
  try {
    ...
    if (!textInput) {
      if (!fromToken || !toToken || !totalAmount) {
        return res.status(400).json({ success: false, error: "inputText or fromToken/toToken/totalAmount required" });
      }
    ...
    const result = await runAgentJob({ startHost: agentHost, purchaseUrl, inputData: { text: textInput }, user: req.user });

    res.json({ success: true, output: { raw: result.result } });
  } catch (error) {
    console.error("Swap token agent error:", error.response?.data || error.message);
    res.status(500).json({ success: false, error: "Failed to execute swap token agent", details: error.response?.data || error.message });
  }
});
...
router.post("/research-agent/invoke", async (req, res) => {
  try {
    const { inputText } = req.body;
    if (!inputText || typeof inputText !== "string") {
      return res.status(400).json({ success: false, error: "inputText is required and must be a string" });
    }

    const agentHost = process.env.AGENT_SERVICE_HOST || "http://139.84.155.199:8000";
    const purchaseUrl = process.env.AGENT_PURCHASE_URL || "http://139.84.155.199:3001/api/v1/purchase/";

    const result = await runAgentJob({ startHost: agentHost, purchaseUrl, inputData: { text: inputText }, user: req.user });

    res.json({ success: true, output: { summary: result.result } });
  } catch (error) {
    console.error("Research agent error:", error.response?.data || error.message);
    res.status(500).json({ success: false, error: "Failed to execute research agent", details: error.response?.data || error.message });
  }
});
...
router.post("/telegram-bot-agent/invoke", async (req, res) => {
  try {
    ...
    console.error("Telegram bot agent error:", error.response?.data || error.message);
    res.status(500).json({ success: false, error: "Failed to execute telegram bot agent", details: error.response?.data || error.message });
  }
});
